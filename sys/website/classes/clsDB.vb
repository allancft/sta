...
#Region "Tratamento Conexão"
    ...
    Private Sub RetornaStringConexao()

        Dim strNomeDB, strNomeServidor, strNomeUsuario, strSenhaUsuario As String
        strNomeServidor = ConfigurationSettings.AppSettings("nomeServidor")
        strNomeDB = ConfigurationSettings.AppSettings("nomeBanco")
        strNomeUsuario = ConfigurationSettings.AppSettings("usuarioNome")
        strSenhaUsuario = ConfigurationSettings.AppSettings("usuarioSenha")

        'Descriptografar senha do banco
        Dim objCriptografia As aim.objCriptografia
        objCriptografia = New aim.objCriptografia
        strSenhaUsuario = objCriptografia.Descriptografar(strSenhaUsuario)
        objCriptografia = Nothing

        'Concatena variável formando string de conexao
        str_auxConexao = "Persist Security Info=False;" & _
                    "Initial Catalog=" & strNomeDB & _
                    ";Data Source=" & strNomeServidor & _
                    ";pwd=" & strSenhaUsuario & _
    ...
    Public Function AbreConexao(Optional ByVal str_conexao As String = "", Optional ByRef obj_conexao As SqlClient.SqlConnection = Nothing, Optional ByVal str_mensagemerroDb As String = "") As Boolean
        ...
        Try
            RetornaStringConexao()
            obj_auxConexao.ConnectionString = str_auxConexao
            obj_auxConexao.Open()
